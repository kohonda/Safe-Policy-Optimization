        ...
        for steps in range(local_steps_per_epoch):
            ...
            for idx, (done, time_out) in enumerate(zip(terminated, truncated)):
                if epoch_end or done or time_out:
                    ...
                    if done or time_out:
                        # Added ====
                        summary_logger.add_scalar(
                            "rollout/episode_reward", ep_ret[idx], global_step
                        )
                        summary_logger.add_scalar(
                            "rollout/episode_cost", ep_cost[idx], global_step
                        )
                        accumulated_cost += ep_cost[idx]
                        summary_logger.add_scalar(
                            "rollout/accumulated_cost", accumulated_cost, global_step
                        )
                        num_episodes += 1
                        summary_logger.add_scalar(
                            "rollout/num_episodes", num_episodes, global_step
                        )
                        print(
